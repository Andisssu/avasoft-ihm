    ...
    <div class="overflow-x-auto w-full">
        ...
        <tbody>
          <tr v-for="patient in filteredPatients" :key="patient.id_patient" class="bg-gray-700 text-white">
            ...
            <td class="py-2 px-4 text-center rounded-r-lg flex justify-center space-x-2">
              ...
              <button class="bg-red-500 text-white py-1 px-2 rounded" @click="openDeleteModal(patient)"
      ...
      <div class="flex justify-end w-full max-w-4xl mt-6">
        <router-link to="/PatientRegister">
          <button class="bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded">Adicionar Paciente</button>
        </router-link>
      </div>
            ...
            <div class="p-4 sm:p-10 overflow-y-auto bg-gray-50">
              <div class="flex gap-x-4 md:gap-x-7">
                ...
                <div class="grow">
                  ...
                  <p class="text-gray-500">Tem certeza que deseja excluir este paciente? Essa ação não poderá ser
                    desfeita.</p>
                </div>
              </div>
            </div>
...
export default {
  ...
  computed: {
    filteredPatients() {
      return this.patients.filter(patient => !this.deletedPatients.includes(patient.id_patient));
    }
  },
  methods: {
    viewDetails(id) {
    ...
    confirmDeletePatient() {
      if (this.patientToDelete) {
        // Adicionar o paciente à lista de excluídos temporariamente
        this.deletedPatients.push(this.patientToDelete.id_patient);
    ...
    updatePatient() {
      // Lógica para atualizar os dados do paciente
      this.showToastMessage('Dados do paciente atualizados com sucesso', 'success');
      this.showModal = false;
    },
  ...
  mounted() {
    ...
    // Inicializar patientToEdit com dados fictícios
    this.patientToEdit = {
      user_id: '1',
